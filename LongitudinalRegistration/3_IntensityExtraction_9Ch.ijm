 ...
 * 
 * This macro expects 9 channel tif: Channel 1 2P timepoint 1, channel 2 MRI timepoint 1, 
 * channel 3 2P timepoint 2, channel 4 MRI timepoint 2, channel 5 2P timepoint 3, 
 * channel 6 MRI timepoint 3, channel 7 2P timepoint 4, channel 8 MRI timepoint 4, 
 * channel 9 mask.
	...
	for (n=0; n<fileList.length; n++) {
		...
		if (endsWith(fileList[n], '.tiff') || endsWith(fileList[n], '.tif')) {
            ...
            if (channels == 9) {
            	...
            	fluoroTitle3 = getTitle();
            	selectImage("C6-" + inputTitle);
            	mriTitle3 = getTitle();
            	selectImage("C7-" + inputTitle);
            	fluoroTitle4 = getTitle();
            	selectImage("C8-" + inputTitle);
            	mriTitle4 = getTitle();
            	selectImage("C9-" + inputTitle);
            	roiTitle = getTitle();
				...
				// Process ch6
				selectImage(mriTitle3);
				PixelIntensityValue(arraySize);
				arraySizeC6 = arraySize;

				arraySize = newArray(width * height * slices);

				// Process ch7
				selectImage(fluoroTitle4);
				PixelIntensityValue(arraySize);
				arraySizeC7 = arraySize;

				arraySize = newArray(width * height * slices);

				// Process ch8
				selectImage(mriTitle4);
				PixelIntensityValue(arraySize);
				arraySizeC8 = arraySize;
				...
				for (l = 0; l < arraySize.length; l++) {
					...
					print(arraySizeC1[l] + "\t" + arraySizeC2[l] + "\t" + arraySizeC3[l] + "\t" + arraySizeC4[l]  + "\t" + arraySizeC5[l] + "\t" + arraySizeC6[l] + "\t" + arraySizeC7[l] + "\t" + arraySizeC8[l] + "\t" + arraySizeC9[l] + "\t" + title + "\t" + (n+1) + "\t" + l + "\t" + info_z[l] + "\t" + info_x[l] + "\t" + info_y[l]);
				...
				exit("Input image must have 9 channels");
