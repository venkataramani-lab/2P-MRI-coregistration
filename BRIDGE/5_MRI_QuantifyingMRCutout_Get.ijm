...
// Lese die Werte aus der vierten Zeile der Datei: Scale
values_scale = split(lines[3], ",");
if (values_scale.length == 3) {
    x_adjust = parseFloat(values_scale[0]);
    y_adjust = parseFloat(values_scale[1]);
    xory = parseFloat(values_scale[2]);
} else {
    waitForUser("Ungültige Zeile in der Datei (Zeile 4): " + lines[3]);
}

// Lese die Werte aus der fünften Zeile der Datei: Crop2
values_crop2 = split(lines[4], ",");
if (values_crop2.length == 4) {
    x_crop2 = parseFloat(values_crop2[0]);
    y_crop2 = parseFloat(values_crop2[1]);
    width_crop2 = parseFloat(values_crop2[2]);
    height_crop2 = parseFloat(values_crop2[3]);
} else {
    waitForUser("Ungültige Zeile in der Datei (Zeile 5): " + lines[4]);
}

// Lese die Werte aus der sechsten Zeile der Datei: z-Anpassung/Duplicate
values_duplicate = split(lines[5], ",");
if (values_duplicate.length == 2) {
    first_z = parseFloat(values_duplicate[0]);
    last_z = parseFloat(values_duplicate[1]);
} else {
    waitForUser("Ungültige Zeile in der Datei (Zeile 6): " + lines[5]);
}
...
waitForUser("Bewege die Linie ein wenig vom Fenster weg.");
...
last_z_2 = first_z_2 + 6;
// SOLL NOCH AUTOMATISIERT WERDEN:
waitForUser("Gib gleich folgendes ein: " + first_z_2 + "-" + last_z_2);
run("Duplicate..."); // 7 Slices auswählen

waitForUser("Falls das Bild zu groß ist, croppe es hier von oben links ausgehend.");
...
waitForUser("Speichere hier originalxy.");
...
// Cropping 2
makeRectangle(x_crop2,y_crop2,width_crop2,height_crop2);
run("Crop");
...
File.append(String.join(linesData_scale, ","), verzeichnis_get);
File.append(String.join(linesData_crop2, ","), verzeichnis_get);

// Schließen der Datei
File.close(file);

waitForUser("Fertig! Speichere hier 7slices. Die Einstellungs-Datei befindet sich hier: " + verzeichnis_get);